...
- **Definitive:** `sync_source.ThreatIntel` is populated. These tables are guaranteed to be IOC indicators; the `indicator_type` matches the `type` column inside the lookup table itself (and the VRL reads `enrichment_result.type` to pick the right typed indicator slot).
...
The canonical reference is `corpus/alienvault_threat_intelligence_enrichment.vrl` — a paste-ready, user-uploadable program. Two things to edit at the top before pasting into the Scanner UI:

- `MATCH_PATH` (array, default `["@ecs", "source", "ip"]`) — the field in the log to read as the candidate IOC value. Switch to `["@ecs", "destination", "ip"]` for outbound-IP matching, `["@ecs", "file", "hash", "sha256"]` for hash-based matching, etc.
- `RESULT_ARRAY_PATH` (array, default `["@ecs", "threat", "enrichments"]`) — where matches get pushed. Stick with the ECS default unless you have a reason not to.

Plus one string literal inside the body to change: the lookup-table name passed to `get_enrichment_table_record(...)`. Scanner's VRL requires that argument to be a string literal (not a variable), so the rename has to happen at the call site. Find the line:
...
and swap the table name to match the lookup table you're joining against (use `scripts/list_lookup_tables.sh --ioc` to see what's available in your tenant).

You **don't** configure the indicator type separately. The program reads `enrichment_result.type` from the matched lookup-table row and uses that single value to populate both the `indicator.type` array AND the typed slot (`indicator.ip` / `indicator.url` / `indicator.email` / `indicator.file.hash.*`). One source of truth, no chance of drift.
...
2. Calls `get_enrichment_table_record("<your-table>", { "indicator": match_value })`.
3. On a hit, reads `enrichment_result.type` (e.g. `"ipv4-addr"`, `"url"`, `"file-hash-sha256"`).
4. Builds a `threat_obj` with `indicator.{name,type,first_seen,last_seen,description,provider,reference}` plus a typed `indicator.{ip|url|email|file.hash.…}` slot keyed off `enrichment_result.type`, and a `matched.{atomic,field}` describing what matched.
5. Pushes onto the existing `@ecs.threat.enrichments` array (preserves prior enrichments).
6. Returns the modified event.
