...
1. **Lookup table of IOCs.** Either synced from a feed (AlienVault OTX, ThreatFox, CISA KEV, Feodo) via a Scanner UI sync source, or uploaded as a CSV (UI: Library → Lookup Tables → +, or the unstable API).
   - **First**, list what's already available: `../write-vrl/scripts/list_lookup_tables.sh --ioc` returns IOC-flavored tables already in the tenant. If a relevant one exists, point at it. (That script also needs `SCANNER_TEAM_ID`, the **Team ID** from **Settings → General**, or the UUID in the settings URL `app.scanner.dev/teams/<TEAM_ID>/settings/overview`. The UI has no "Tenant ID" field, so say that outright rather than sending the user browsing Settings; the older `SCANNER_TENANT_ID` still works as a fallback. If neither is set, skip the survey and keep going.)
...
2. **VRL enrichment** (`/write-vrl`): joins the log's relevant field (`@ecs.source.ip`, `@ecs.destination.domain`, `@ecs.file.hash.sha256`, etc.) against the lookup table at ingest time, writing matches into the `@ecs.threat.enrichments` array. Canonical example: `skills/write-vrl/corpus/alienvault_threat_intelligence_enrichment.vrl`; the full pattern is documented in `skills/write-vrl/references/ioc_enrichment.md`.
