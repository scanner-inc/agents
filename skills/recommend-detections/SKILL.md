...
- `SCANNER_API_URL`, `SCANNER_API_KEY`, `SCANNER_TEAM_ID` — for the Detection Rules REST API (reusing the `scripts/list_detection_rules.sh` script from `posture-report`). `SCANNER_API_URL` / `SCANNER_API_KEY` come from **Settings → API Keys**. `SCANNER_TEAM_ID` is the **Team ID** (**Settings → General**, or the UUID in the settings URL `app.scanner.dev/teams/<TEAM_ID>/settings/overview`); the API currently says `tenant_id`, the UI says Team ID, and there is no "Tenant ID" field in the product, so answer that directly instead of sending the user browsing Settings. The script still reads the older `SCANNER_TENANT_ID` as a fallback, but tell users to set `SCANNER_TEAM_ID`.
