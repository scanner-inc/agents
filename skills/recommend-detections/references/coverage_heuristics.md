...
3. **Concrete-mismatch suspect.** The rule's `@scnr.source_type` filter references a source-type the tenant does **not** ingest, *or* the filter clearly can't match the schema of the source it claims to target. This is real breakage — recommend re-mapping or deletion.
