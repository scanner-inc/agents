...
8. **Seed inline tests** from source-repo fixtures where available (most repos ship example events). Translate into Scanner `dataset_inline` JSONL, setting `dataset_format` explicitly (`RawJson` for raw nested events, `FlatTable` for flat column tables) and giving each event an `@scnr.datetime` timestamp (nested `"@scnr": {"datetime": ...}` under `RawJson`; flat key under `FlatTable`). Panther test fixtures are Python dicts; the model converts.
