# Canonical MITRE tags supported by Scanner

This is the source-of-truth list (mirrored from `~/src/web-client/src/util/mitre_tags.ts`, 204 entries: 14 tactics + 190 techniques). When tagging a detection rule, use these **exact strings**. Don't invent variants.

Cite tags by canonical ID. Never use display names like "Defense Evasion" — always `tactics.ta0005.defense_evasion`.
...
## Other Scanner-supported tag conventions

When the canonical MITRE list above doesn't fit, use these in addition:

- `source.<source-slug>` — log-source provenance. Examples: `source.cloudtrail`, `source.okta`, `source.github`, `source.gsuite`, `source.azure`, `source.gcp`. Always pair with at least one MITRE tag where the rule has tactical intent.
- `correlation.<custom-label>` — marks a rule as a participant in a custom correlation. The `write-correlation` skill writes this tag onto constituent rules so a correlation rule can join them via `tags[*]:correlation.<label>`.
- `scanner.<bucket>` — Scanner-internal observability rules (e.g., `scanner.querying`, `scanner.integration_access`). Not for customer detections.
- Custom organization tags — e.g., `team.security_engineering`, `compliance.pci`, `priority.tier_1`. Use these freely; Scanner tag rules only require letter-start + ASCII alphanumeric + `.`, `-`, `_`.
