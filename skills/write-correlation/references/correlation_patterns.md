...
Replace `00000000-…` with the real `_detections` UUID resolved via the MCP query above (`@index=_detections | head 1 | table(@index, @index_id)` — the `@index_id` field is the UUID).
